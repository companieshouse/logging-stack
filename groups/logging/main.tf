...
module "elasticsearch" {
  ...
  elastic_search_client_cidrs   = local.administration_cidrs
...
locals {
  ...
  subnet_id = data.terraform_remote_state.networking.outputs.management_private_subnet_ids["${var.region}a"]
  vpc_id = data.terraform_remote_state.networking.outputs.management_vpc_id
}
